...
# ARG1 is the path and ARG3 is the type of run (sh, dh, indedh)
...
set output ARG1."/load_%".ARG3.".eps"


if (ARG2 eq "dh") {
    plot ARG1.'/percent'.ARG3 using 0:1 w lp lw 2 title "server 0",\
    ARG1.'/percent'.ARG3 using 0:2 w lp lw 2 title "server 1",\
    ARG1.'/percent'.ARG3  using 0:3 w lp lw 2 title "server 2",\
    ARG1.'/percent'.ARG3  using 0:4 w lp lw 2 title "server 3",\
...
if (ARG2 eq "indedh" || ARG3 eq "windowed") {
    plot ARG1.'/percent'.ARG3  using 0:1 w lp lw 2 title "server 0",\
    ARG1.'/percent'.ARG3  using 0:2 w lp lw 2 title "server 1",\
    ARG1.'/percent'.ARG3  using 0:3 w lp lw 2 title "server 2",\
    ARG1.'/percent'.ARG3  using 0:4 w lp lw 2 title "server 3",\
    ARG1.'/rebalancing'.ARG3  using 1:(ymax):1 w impulse title "rebalancing" lt 3 lc rgb "violet"
}
